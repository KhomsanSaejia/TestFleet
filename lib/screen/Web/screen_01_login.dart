...
import 'package:fleetdemo/model/model_user.dart';
import 'package:fleetdemo/screen/Web/screen_02_home.dart';
...
class ScreenLoginWeb extends StatefulWidget {
  const ScreenLoginWeb({Key? key}) : super(key: key);

  @override
  _ScreenLoginWebState createState() => _ScreenLoginWebState();
}

class _ScreenLoginWebState extends State<ScreenLoginWeb> {
                    ...
                    children: <Widget>[
                      ...
                      InkWell(
                        ...
                        child: Container(
                          ...
                          child: Center(
                              child: Mystyle().textHeader("เข้าสู่ระบบ Web")),
                        ),
                      )
                    ],
                  ),
  ...
  Future<void> checkAuthen() async {
    ...
    await Dio().get(MyConstant().urlCheckUser + username!).then((value) {
      if (value.toString() != 'null') {
        for (var map in value.data) {
          print(map);
        }
